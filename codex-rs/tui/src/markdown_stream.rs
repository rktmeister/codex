...
//!
//! `MarkdownStreamCollector` buffers incoming token deltas and exposes a stable commit boundary.
//! The stream controllers (`streaming/controller.rs`) call `commit_complete_source()` after each
//! newline-bearing delta to obtain the completed prefix for re-rendering, leaving the trailing
//! incomplete line or markdown block in the buffer for the next delta.
//!
...
/// Newline-gated accumulator that buffers raw markdown source and commits only stable source.
///
...
///
/// The collector does not render markdown in production. It only defines stable source boundaries;
...
impl MarkdownStreamCollector {
    ...
    /// Commit newly completed raw markdown source up to the last stable boundary.
    ///
    /// This returns the range of source that has not been returned by a previous commit. Calling it
    /// after a delta without a newline returns `None`, as does a delta that only completes lines
    /// inside an open markdown block that may change meaning when more source arrives.
    ...
    pub fn commit_complete_lines(&mut self) -> Vec<Line<'static>> {
        ...
        self.commit_lines_to(commit_end)
    }

    #[cfg(test)]
    fn commit_stable_lines(&mut self) -> Vec<Line<'static>> {
    ...
    #[cfg(test)]
    fn commit_lines_to(&mut self, commit_end: usize) -> Vec<Line<'static>> {
...
pub(crate) fn simulate_stream_markdown_for_tests(
    ...
    for d in deltas {
        ...
        if d.contains('\n') {
            out.extend(collector.commit_stable_lines());
        }
    }
    if finalize {

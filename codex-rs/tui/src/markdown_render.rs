    ...
    I: Iterator<Item = (Event<'a>, Range<usize>)>,
{
    ...
    pending_callout_marker: Option<String>,
    ...
    I: Iterator<Item = (Event<'a>, Range<usize>)>,
{
    ...
    fn start_blockquote(&mut self) {
        ...
        self.indent_stack.push(
            ...
            .with_style(self.styles.blockquote),
        );
    }
    ...
    fn end_codeblock(&mut self) {
        ...
        if let Some(lang) = self.code_block_lang.take() {
            ...
            let available_width = self.wrap_width.map(|width| {
                width.saturating_sub(Self::line_display_width(&Line::from(
    ...
    fn maybe_apply_callout_marker(&mut self, text: CowStr<'a>) -> CowStr<'a> {
        if let Some(marker) = self.pending_callout_marker.take() {
            let mut combined = marker;
            combined.push_str(text.as_ref());
            return self.consume_callout_marker_candidate(CowStr::from(combined));
        }

        let Some(current_line) = self.current_line_content.as_ref() else {
            return text;
        };
        if !current_line.line.spans.is_empty() {
            return text;
        }
        if !self.has_callout_context() {
            return text;
        }
        self.consume_callout_marker_candidate(text)
    }

    fn consume_callout_marker_candidate(&mut self, text: CowStr<'a>) -> CowStr<'a> {
        let Some((label, style, rest)) = self.parse_callout_marker(text.as_ref()) else {
            if self.is_callout_marker_prefix(text.as_ref()) {
                self.pending_callout_marker = Some(text.to_string());
                return CowStr::from("");
            }
            return text;
        };
        let rest = rest.to_string();
        if !self.apply_callout_context(style) {
            return text;
        }
    ...
    fn has_callout_context(&self) -> bool {
        self.indent_stack
            .iter()
            .rev()
            .any(|ctx| ctx.style.is_some())
    }

    fn apply_callout_context(&mut self, style: Style) -> bool {
        ...
        context.prefix = vec![Span::styled("│ ", style)];
        true
    }

    fn flush_pending_callout_marker(&mut self) {
        let Some(marker) = self.pending_callout_marker.take() else {
            return;
        };
        if marker.is_empty() {
            return;
        ...
        let style = self.inline_styles.last().copied().unwrap_or_default();
        self.push_span(Span::styled(marker, style));
    }

    fn is_callout_marker_prefix(&self, text: &str) -> bool {
        !text.is_empty()
            && [
                "[!NOTE]",
                "[!TIP]",
    ...
    fn apply_callout_header(&mut self, callout: PendingCallout) {
        ...
        let mut initial_indent = self.prefix_spans(/*pending_marker_line*/ false);
    ...
    fn push_prewrapped_line(&mut self, mut line: HyperlinkLine, pending_marker_line: bool) {
        self.flush_current_line();
        let style = self.active_indent_style().unwrap_or(line.line.style);

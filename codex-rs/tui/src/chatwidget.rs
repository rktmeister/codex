...
use codex_git_utils::GitLineStats;
use codex_git_utils::branch_diff_line_counts;
...
pub(crate) struct ChatWidget {
    ...
    copy_code_binding: Vec<KeyBinding>,
...
impl ChatWidget {
    ...
    fn copy_text_to_clipboard_with(
        ...
        match copy_fn(text) {
            Ok(lease) => {
                ...
                self.add_to_history(history_cell::new_info_event(success_message, hint));
            }
    ...
    fn open_copy_code_popup(&mut self) {
        let Some(text) = self.transcript.last_agent_markdown.as_deref() else {
        ...
        let copy_hint = self.turn_lifecycle.agent_turn_running.then_some(

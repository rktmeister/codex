...
impl Config {
    ...
    pub fn resolve_py_repl_python_path(&self) -> Option<PathBuf> {
        std::env::var_os("CODEX_PY_REPL_PYTHON_PATH")
            .map(PathBuf::from)
            .or_else(|| self.py_repl_python_path.clone())
    }

    /// Resolve the configured import search roots for `py_repl`.
    ///
    /// The environment variable takes precedence so test harnesses and wrappers
    /// can inject package roots without mutating user config.
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let py_repl_python_path = cfg.py_repl_python_path.map(Into::into);
        let py_repl_sys_path = cfg
            .py_repl_sys_path

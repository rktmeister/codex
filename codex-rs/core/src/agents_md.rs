...
fn render_py_repl_instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- `codex.process.python(...)` runs code in a fresh Python child process using the active py_repl interpreter by default, with optional `args`, `interpreter`, `cwd`, `env`, and timeout/permission options.\n");

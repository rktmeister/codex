...
pub(crate) async fn discover_project_root(
    ...
    let dir = normalized_config_cwd(config)?;
...
fn normalized_config_cwd(config: &Config) -> io::Result<AbsolutePathBuf> {
...
impl<'a> AgentsMdManager<'a> {
    ...
    pub(crate) async fn user_instructions_with_fs(
        ...
        if let Some(py_repl_section) = render_py_repl_instructions(self.config) {
            if !output.is_empty() {
                output.push_str("\n\n");
            }
            output.push_str(&py_repl_section);
        }
    ...
    async fn agents_md_paths(
        ...
        let dir = normalized_config_cwd(self.config)?;
        let project_root = discover_project_root(self.config, fs).await?;

...
fn render_py_repl_instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- Helpers: `codex.tmp_dir`, `codex.runtime_info()`, `codex.process.run(...)`, `codex.tool(name, args=None)`, `codex.emit_image(image_like)`, and `codex.emitImage(image_like)`.\n");

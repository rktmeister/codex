...
pub(crate) async fn discover_project_root(
    ...
    discover_project_root_for_cwd(config, &PathUri::from_abs_path(&config.cwd), fs)
        .await?
        .to_abs_path()
}

async fn discover_project_root_for_cwd(
    config: &Config,
    cwd: &PathUri,
    fs: &dyn ExecutorFileSystem,
) -> io::Result<PathUri> {
...
fn render_py_repl_instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- `py_repl` is a freeform/custom tool. Direct `py_repl` calls must send raw Python tool input (optionally with first-line `# codex-py-repl: timeout_ms=15000 isolated=true`). Do not wrap code in JSON (for example `{\"code\":\"...\"}`), quotes, or markdown code fences.\n");
    section.push_str("- Helpers: `codex.tmp_dir`, `codex.runtime_info()`, `codex.process.run(...)`, `codex.process.start(...)`, `codex.process.python(...)`, `codex.tool(name, args=None)`, `codex.emit_image(image_like)`, and `codex.emitImage(image_like)`.\n");
    section.push_str("- `codex.tool(...)` starts a nested tool call and returns the raw tool response item dict. Await it when you need the result; nested tool outputs stay inside Python unless you print or emit them.\n");
    section.push_str("- `codex.process.run(...)` starts a host-mediated subprocess with per-call `cwd`, `env`, `timeout_ms`, and permission options; prefer an argv list such as `[sys.executable, \"-c\", \"...\"]` when shell parsing is not needed.\n");
    section.push_str("- `codex.process.start(...)` starts a long-running host-mediated subprocess and returns a handle with `poll(...)`, `wait(...)`, `kill()`, `head(...)`, `tail(...)`, and cumulative `output_path`; use `codex.process.list()`, `codex.process.kill(session_id)`, and `codex.process.kill_all()` to clean up tracked children. Use `codex.process.on_failure(callback)` for cleanup callbacks that should run when a cell fails.\n");
    section.push_str("- `codex.process.python(...)` runs code in a fresh Python child process using the active py_repl interpreter by default, with optional `args`, `interpreter`, `cwd`, `env`, and timeout/permission options; use `codex.process.use_python(path)` or `codex.process.use_venv(path)` to change that default for later calls.\n");
...
pub(crate) async fn load_project_instructions(
    ...
    if let Some(py_repl_section) = render_py_repl_instructions(config) {
        loaded.entries.push(InstructionEntry {
            contents: py_repl_section,
...
async fn agents_md_paths(
    ...
    let dir = cwd.clone();
    let project_root = discover_project_root_for_cwd(config, cwd, fs).await?;

    let search_dirs: Vec<PathUri> = if project_root != dir {

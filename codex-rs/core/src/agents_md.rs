...
fn render_py_repl_instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- `codex.process.start(...)` starts a long-running host-mediated subprocess and returns a handle with `poll(...)`, `wait(...)`, and `kill()`; use `codex.process.list()`, `codex.process.kill(session_id)`, and `codex.process.kill_all()` to clean up tracked children.\n");

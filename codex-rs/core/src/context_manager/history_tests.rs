...
#[test]
fn record_items_truncates_code_mode_text_output_and_preserves_image_content() {
    let mut history = ContextManager::new();
    ...
    let output = ResponseItem::CustomToolCallOutput {
        ...
        output: FunctionCallOutputPayload::from_content_items(vec![
            ...
            FunctionCallOutputContentItem::InputImage {
                image_url: "https://example.com/code-mode-result.png".to_string(),
                detail: Some(DEFAULT_IMAGE_DETAIL),
            },
        ]),
        internal_chat_message_metadata_passthrough: None,
    ...
    match &history.items[1] {
        ResponseItem::CustomToolCallOutput { output, .. } => {
            let FunctionCallOutputBody::ContentItems(items) = &output.body else {
                panic!("expected content items output");
            };
            assert_eq!(
                items,
                &vec![
                    FunctionCallOutputContentItem::InputText {
                        text: truncate_text(&long_output, policy * 1.2),
                    },
                    FunctionCallOutputContentItem::InputImage {
                        image_url: "https://example.com/code-mode-result.png".to_string(),
                        detail: Some(DEFAULT_IMAGE_DETAIL),

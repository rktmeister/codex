...
fn expected_js_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        ...
        "- `codex.emitImage(...)` adds one image to the outer `js_repl` function output each time you call it, so you can call it multiple times to emit multiple images. It accepts a base64 `data:` URL for a PNG, JPEG, GIF, or WebP image, a single `input_image` item, an object like `{ bytes, mimeType }`, or a raw tool response object with exactly one image and no text. It rejects mixed text-and-image content. Unsupported formats such as SVG are rejected.\n",
...
fn expected_py_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        ...
        "- Helpers: `codex.tmp_dir`, `codex.tool(name, args=None)`, `codex.emit_image(image_like)`, and `codex.emitImage(image_like)`.\n",
        "- `codex.tool(...)` starts a nested tool call. Await it when you need the result; nested tool outputs stay inside Python unless you print or emit them.\n",
...
async fn js_repl_instructions_are_appended_when_enabled() {
    ...
    assert_eq!(res, expected_js_repl_instructions(/*tools_only*/ false));
}
...
async fn js_repl_tools_only_instructions_are_feature_gated() {
    ...
    let res = get_user_instructions(&cfg)
        .await
        .expect("js_repl instructions expected");
    assert_eq!(res, expected_js_repl_instructions(/*tools_only*/ true));
}

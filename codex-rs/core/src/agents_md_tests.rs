...
fn expected_py_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        ...
        "- `codex.process.start(...)` starts a long-running host-mediated subprocess and returns a handle with `poll(...)`, `wait(...)`, and `kill()`; use `codex.process.list()`, `codex.process.kill(session_id)`, and `codex.process.kill_all()` to clean up tracked children. Use `codex.process.on_failure(callback)` for cleanup callbacks that should run when a cell fails.\n",
        "- `codex.process.python(...)` runs code in a fresh Python child process using the active py_repl interpreter by default, with optional `args`, `interpreter`, `cwd`, `env`, and timeout/permission options.\n",

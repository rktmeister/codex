...
fn expected_py_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        ...
        "- `codex.process.start(...)` starts a long-running host-mediated subprocess and returns a handle with `poll(...)`, `wait(...)`, and `kill()`; use `codex.process.list()`, `codex.process.kill(session_id)`, and `codex.process.kill_all()` to clean up tracked children.\n",

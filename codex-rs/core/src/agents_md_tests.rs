...
fn expected_py_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        ...
        "- Helpers: `codex.tmp_dir`, `codex.runtime_info()`, `codex.process.run(...)`, `codex.process.python(...)`, `codex.tool(name, args=None)`, `codex.emit_image(image_like)`, and `codex.emitImage(image_like)`.\n",

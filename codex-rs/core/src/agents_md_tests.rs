...
async fn project_doc_user_instructions(config: &TestConfig) -> Option<String> {
...
async fn no_doc_file_returns_none() {
    ...
    let res = project_doc_user_instructions(
...
async fn doc_larger_than_limit_is_truncated() {
    ...
    let res = project_doc_user_instructions(&make_config(&tmp, LIMIT, /*instructions*/ None).await)
        .await
        .expect("doc expected");
...
async fn finds_doc_in_repo_root() {
    ...
    let res = project_doc_user_instructions(&cfg)
        .await
        .expect("doc expected");
...
async fn zero_byte_limit_disables_docs() {
    ...
    let res =
        project_doc_user_instructions(&make_config(&tmp, /*limit*/ 0, /*instructions*/ None).await)
            .await;
    assert!(
        res.is_none(),
...
fn expected_py_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        "## Python REPL\n",
        "- Use `py_repl` for Python with top-level await in a persistent kernel.\n",
        ...
        "- `py_repl` is a freeform/custom tool. Direct `py_repl` calls must send raw Python tool input (optionally with first-line `# codex-py-repl: timeout_ms=15000 isolated=true`). Do not wrap code in JSON (for example `{\"code\":\"...\"}`), quotes, or markdown code fences.\n",
        "- Helpers: `codex.tmp_dir`, `codex.runtime_info()`, `codex.process.run(...)`, `codex.process.start(...)`, `codex.process.python(...)`, `codex.tool(name, args=None)`, `codex.emit_image(image_like)`, and `codex.emitImage(image_like)`.\n",
        "- `codex.tool(...)` starts a nested tool call and returns the raw tool response item dict. Await it when you need the result; nested tool outputs stay inside Python unless you print or emit them.\n",
        "- `codex.process.run(...)` starts a host-mediated subprocess with per-call `cwd`, `env`, `timeout_ms`, and permission options; prefer an argv list such as `[sys.executable, \"-c\", \"...\"]` when shell parsing is not needed.\n",
        "- `codex.process.start(...)` starts a long-running host-mediated subprocess and returns a handle with `poll(...)`, `wait(...)`, `kill()`, `head(...)`, `tail(...)`, and cumulative `output_path`; use `codex.process.list()`, `codex.process.kill(session_id)`, and `codex.process.kill_all()` to clean up tracked children. Use `codex.process.on_failure(callback)` for cleanup callbacks that should run when a cell fails.\n",
        "- `codex.process.python(...)` runs code in a fresh Python child process using the active py_repl interpreter by default, with optional `args`, `interpreter`, `cwd`, `env`, and timeout/permission options; use `codex.process.use_python(path)` or `codex.process.use_venv(path)` to change that default for later calls.\n",
    ...
    if tools_only {
        expected.push_str(
            "- Do not call tools directly; use `py_repl` + `codex.tool(...)` for all tool calls, including shell commands.\n",
        );
        expected
...
#[tokio::test]
async fn py_repl_instructions_are_appended_when_enabled() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let mut cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    cfg.features
        .enable(Feature::PyRepl)
        .expect("test config should allow py_repl");

    let res = get_user_instructions(&cfg)
        .await
        .expect("py_repl instructions expected");
    assert_eq!(res, expected_py_repl_instructions(/*tools_only*/ false));
}

#[tokio::test]
async fn py_repl_tools_only_instructions_are_feature_gated() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let mut cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    let mut features = cfg.features.get().clone();
    features
        .enable(Feature::PyRepl)
        .enable(Feature::PyReplToolsOnly);
    cfg.features
        .set(features)
        .expect("test config should allow py_repl tool restrictions");

    let res = get_user_instructions(&cfg)
        .await
        .expect("py_repl instructions expected");
    assert_eq!(res, expected_py_repl_instructions(/*tools_only*/ true));
}
...
async fn merges_existing_instructions_with_agents_md() {
    ...
    let res =
        project_doc_user_instructions(&make_config(&tmp, /*limit*/ 4096, Some(INSTRUCTIONS)).await)
            .await
...
async fn apps_feature_does_not_emit_user_instructions_by_itself() {
    ...
    let res = project_doc_user_instructions(&cfg).await;
    assert_eq!(res, None);
}

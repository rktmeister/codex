...
fn expected_py_repl_instructions(tools_only: bool) -> String {
    let mut expected = concat!(
        ...
        "- `py_repl` is a freeform/custom tool. Direct `py_repl` calls must send raw Python tool input (optionally with first-line `# codex-py-repl: timeout_ms=15000`). Do not wrap code in JSON (for example `{\"code\":\"...\"}`), quotes, or markdown code fences.\n",

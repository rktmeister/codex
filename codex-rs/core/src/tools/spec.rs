...
pub(crate) fn build_specs_with_discoverable_tools(
    ...
    for handler in plan.handlers {
        match handler.kind {
            ...
            ToolHandlerKind::PyRepl => {
                builder.register_handler(handler.name, py_repl_handler.clone());
            }
            ToolHandlerKind::PyReplReset => {
                builder.register_handler(handler.name, py_repl_reset_handler.clone());
            }

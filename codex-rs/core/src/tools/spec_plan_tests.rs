...
#[tokio::test]
async fn py_repl_enabled_adds_tools() {
    let plan = probe(|turn| {
        set_feature(turn, Feature::PyRepl, /*enabled*/ true);
    })
    .await;

    plan.assert_visible_contains(&["py_repl", "py_repl_reset"]);
    plan.assert_registered_contains(&["py_repl", "py_repl_reset"]);
}
#[tokio::test]

...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    assert!(
        has_path_param(&dot_git_canonical.join("config")),
        "missing .git/config carveout: {args:#?}"
    );
    assert!(
        has_path_param(&dot_git_canonical.join("hooks")),
        "missing .git/hooks carveout: {args:#?}"
    );

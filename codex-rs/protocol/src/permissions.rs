...
fn add_dynamic_git_metadata_entries(entries: &mut Vec<ResolvedFileSystemEntry>) {
    let explicit_entry_count = entries.len();
    ...
    for git_admin_root in dedup_git_admin_roots(git_admin_roots) {
        if !entries[..explicit_entry_count]
            .iter()
            .any(|entry| entry.path == git_admin_root.path && !entry.access.can_write())
        {
...
mod tests {
    ...
    fn writable_roots_protect_worktree_gitdir_and_commondir() {
        ...
        fs::create_dir_all(&commondir).expect("create commondir");
        fs::write(
        ...
        let commondir = AbsolutePathBuf::from_absolute_path(
            commondir.canonicalize().expect("canonicalize commondir"),
        )
        .expect("absolute commondir");

        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry {
            path: FileSystemPath::Path {
                path: worktree_root.clone().into(),
            },
            access: FileSystemAccessMode::Write,
            missing_path_behavior: None,
        }]);
        ...
        let worktree_writable_root = writable_roots
            .iter()
    ...
    fn with_additional_legacy_workspace_writable_roots_protects_metadata() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Path {
                        path: extra.join(".git/config").into()
                    },
                    FileSystemAccessMode::Read,
                ),
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Path {
                        path: extra.join(".git/hooks").into()
                    },
                    FileSystemAccessMode::Read,
                ),
            ])

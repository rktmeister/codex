...
- `codex.emitImage(...)` accepts a base64 `data:` URL for a supported raster image (`image/png`, `image/jpeg`, `image/gif`, or `image/webp`), a single `input_image` item, an object like `{ bytes, mimeType }`, or a raw tool response object that contains exactly one image and no text. Call it multiple times if you want to emit multiple images.
